...
void insert(HashTable *hashTable, int key, int data) {
    ...
    int i = 0;
    while (hashTable->items[(index + i)%CAPACITY].key != dummyData.key && hashTable->items[(index + i) % CAPACITY].key != key){
        i++;
        if (i == CAPACITY) return;
    }
    int targetIndex = (index + i) % CAPACITY;
    if (hashTable->items[targetIndex].key == key) {
        hashTable->items[targetIndex].data = data;
        return;
    }
    hashTable->items[targetIndex] = createHashItem(key, data);
    hashTable->size++;
}
